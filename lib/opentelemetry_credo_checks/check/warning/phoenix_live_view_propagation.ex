defmodule OpentelemetryCredoChecks.Check.Warning.PhoenixLiveViewProcessPropagation do
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      All Phenix.LiveView async macro calls should be replaced with calls to `OpentelemetrexPhoenix.LiveView` async macros.
      """
    ]
  ...
  defp traverse(
    ...
    aliased_process_propagator? = "OpentelemetrexPhoenix.LiveView" in aliases
  ...
  defp issue_for(issue_meta, meta, trigger) when trigger in @phoenix_liveview_async do
    format_issue(
      issue_meta,
      message:
        "There should be no calls to `#{trigger}`, `OpentelemetrexPhoenix.LiveView.#{trigger}` must be used instead.",
      trigger: trigger,
      line_no: meta[:line],
      column: meta[:column]
    )
